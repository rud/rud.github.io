...
[`/r/fpv`](https://reddit.com/r/fpv) has been a helpful community in getting started and finding great gear that works together.
...
I picked a momentary switch (one that outomatically switches to OFF after clicking it) on the opposite side of the radio from the ARM toggle, just to make sure I need to use both hands to arm the drone.
  ...
  1. Find "PREARM" in the list of modes in betaflight, click "Range", pick "AUTO" as the input, then toggle the switch on the radio you want to serve as your pre-arm switch. In this setup I use the momentary switch `SF` on the RadioMasterBoxer. The "AUTO" will now automatically switch to the actual button identity. Turns out this is `AUX6` for this particular radio.
...
* Hold the `SF` momentary button (meaning it's spring-loaded and automatically switches off.
